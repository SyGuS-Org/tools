...
namespace SynthLib2Parser {
    ...
    void FunDefCmd::SetFunName(string s)
    {
      Symbol = s;
    }
    ...
    SynthFunCmd::SynthFunCmd(const SourceLocation& Location,
                             ...
                             SymbolTableScope* Scope)
        : ASTCmd(Location, CMD_SYNTHFUN), SynthFunName(Name),
          Arguments(Args), FunType(FunType), Scope(Scope)
    {
        // Nothing here
    }
    ...
    SynthInvCmd::SynthInvCmd(const SourceLocation& Location,
                             ...
                             const vector<NTDef*> GrammarRules,
                             SymbolTableScope* Scope)
        : ASTCmd(Location, CMD_SYNTHINV), SynthInvName(Name),
          Arguments(Args), GrammarRules(GrammarRules),
          Scope(Scope)
    {
        // Nothing here
    }
    ...
    SynthInvCmd::SynthInvCmd(const SourceLocation& Location,
                             ...
                             SymbolTableScope* Scope)
        : ASTCmd(Location, CMD_SYNTHINV), SynthInvName(Name),
          Arguments(Args), Scope(Scope)
    {
        // Nothing here
    }


    SynthInvCmd::~SynthInvCmd()
    {
        for(auto const& ASPair : Arguments) {
            delete ASPair;
        }
    ...
    void SynthInvCmd::Accept(ASTVisitorBase* Visitor) const
    {
    ...
    {
        return new SynthInvCmd(Location, SynthInvName, CloneVector(Arguments),
                               CloneVector(GrammarRules), Scope->Clone());
    }

    const string& SynthInvCmd::GetInvName() const
    {
        return SynthInvName;
    }

    const ArgList& SynthInvCmd::GetArgs() const
    {
        return Arguments;
    }


    const vector<NTDef*>& SynthInvCmd::GetGrammarRules() const
    {
        return GrammarRules;
    }
    
    void SynthInvCmd::SetScope(SymbolTableScope* Scope) const
    {
        this->Scope = Scope;
    }

    SymbolTableScope* SynthInvCmd::GetScope() const
    {
        return Scope;
    }
    ...
    InvConstraintCmd::InvConstraintCmd(const SourceLocation& Location, 
                                       const string& SynthInvName, const string& PreCondName,
                                       const string& TransName, const string& PostCondName)
        : ASTCmd(Location, CMD_INV_CONSTRAINT), SynthInvName(SynthInvName),
          PreCondName(PreCondName), TransName(TransName), PostCondName(PostCondName)
    {
        // Nothing here
    }
    ...
    {
        return new InvConstraintCmd(Location, SynthInvName, PreCondName, TransName, PostCondName);
    }

    const string& InvConstraintCmd::GetSynthInvName() const
    {
          return SynthInvName;
    }

    const string& InvConstraintCmd::GetPreCondName() const
    {
          return PreCondName;
    }

    const string& InvConstraintCmd::GetTransName() const
    {
          return TransName;
    }

    const string& InvConstraintCmd::GetPostCondName() const
    {
          return PostCondName;
    }
    ...
    PrimedVarDeclCmd::PrimedVarDeclCmd(const SourceLocation& Location,
                           const string& VarName,
                           const SortExpr* VarType)
        : ASTCmd(Location, CMD_PRIMEDVARDECL), 
          VarName(VarName), PrimedVarName(VarName+"!"),
          VarType(VarType)
    {
        // Nothing here
    }
    ...
    {
        return new PrimedVarDeclCmd(Location, VarName, static_cast<const SortExpr*>(VarType->Clone()));
    }

    const string& PrimedVarDeclCmd::GetName() const
    {
        return VarName;
    }

    const string& PrimedVarDeclCmd::GetPrimedName() const
    {
        return PrimedVarName;
    }
    ...
    StringSortExpr::StringSortExpr(const SourceLocation& Location)
        : SortExpr(Location, SORTKIND_STRING)
    {
        // Nothing here
    }

    StringSortExpr::StringSortExpr()
        : SortExpr(SourceLocation::None, SORTKIND_STRING)
    {
        // Nothing here
    }
    ...
    bool StringSortExpr::Equals(const SortExpr& Other) const
    {
        auto OtherPtr = dynamic_cast<const StringSortExpr*>(&Other);
        if(OtherPtr == NULL) {
            return false;
        } else {
            return true;
    ...
    u32 StringSortExpr::Hash() const
    {
        return boost::hash_value((u32)GetKind());
    }
    
    void StringSortExpr::Accept(ASTVisitorBase* Visitor) const
    {
        Visitor->VisitStringSortExpr(this);
    }
    
    ASTBase* StringSortExpr::Clone() const
    {
        return new StringSortExpr(Location);
    }

    string StringSortExpr::ToMangleString() const
    {
        return "String";
    }
    ...
    const SortExpr* FunTerm::GetTermSort(SymbolTable* SymTab) const 
    {
        ...
        /*
        if(Entry == NULL) {
	    string* s = const_cast<string*>(&FunName);
            *s = "_" + FunName;
            Entry = SymTab->LookupFun(FunName, ArgSorts);
        }*/
        ...
        auto EntryInv = SymTab->LookupInv(FunName);
        if(EntryInv == NULL) {
            Entry = SymTab->LookupInv(FunName);
        }
    ...
    const SortExpr* LetTerm::GetTermSort(SymbolTable* SymTab) const 
    {
        ...
        for(auto const& Binding : Bindings) {
	    SymTab->BindLetVariable(Binding->GetVarName(),
                                    static_cast<Term*>(Binding->GetBoundToTerm()->Clone()));

            // Check that the sort of the term is what is expected
            auto TermSort = Binding->GetBoundToTerm()->GetTermSort(SymTab);
            auto ExpectedSort = Binding->GetVarSort();
            if (!SymTab->ResolveSort(TermSort)->Equals(*(SymTab->ResolveSort(ExpectedSort)))) {
    ...
    const SortExpr* FunGTerm::GetTermSort(SymbolTable* SymTab) const 
    {
        ...
        auto  EntryInv = SymTab->LookupInv(FunName);
        if(EntryInv != NULL) {
            throw SynthLib2ParserException("Identifier \"" + FunName + "\" is " + 
                                           "an invariant which cannot be explicitly referred to.");
        }
    ...
    void ASTVisitorBase::VisitStringSortExpr(const StringSortExpr* Sort)
    {
        // Nothing to do really
    }
    ...
    SynthLib2Parser::SynthLib2Parser(SymbolTable* SymTbl)
        : TheProgram(NULL), TheSymbolTable(SymTbl),
          ParseComplete(false)
    {
        // Nothing here
    }    
                                       ...
                                       bool Pedantic)
    {
        ...
        // Sometime we want to start with a given table
        // For instance, when postprocessing solver-solution
        // we want it to start with the table created when 
        // parsing the benchmark (minus the synthesized funcs)
        // so that it gets the user defined funcs
        if(TheSymbolTable == NULL) {
        ...
        // Don't reset, in the case above we do want parametrs
        // such as BVLoaded to retain their value
