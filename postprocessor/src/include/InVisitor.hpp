#if !defined __SYNTHLIB2_PARSER_IN_VISITOR_HPP
#define __SYNTHLIB2_PARSER_IN_VISITOR_HPP
...
namespace SynthLib2Parser {
    ...
    class InVisitor : public ASTVisitorBase
    {
    private:
        u32 IndentLevel;
        ...
        vector<const ASTCmd*> Cmds;
        vector<string>        CmdAuxString;

        bool        InConstraint;
        bool        InConstraintEquiv;
        bool        SynthFunFound;
        set<string> ForallVars; 
        string      Logic; /* Logic command to output */
        string      LogicName; /* Logic name */
        u32         NumSynthCmds;
        bool        SyntacticRestrictionsGiven;
        bool        InvConstraintCmdSpecified;
        bool        SynthInvGiven;
        bool        NonExampleConstraint; 
        CompTrack   Track;


   public:
      InVisitor(ostream& Out, SymbolTable* SymTbl, CompTrack);
      virtual ~InVisitor();
      
      const ASTCmd* GetCmd(u32 i);
      void SetCmd(u32 i, const ASTCmd* c);
      u32 CmdsSize();
      string GetCmdAuxString(u32 i);
      string GetLogic() { return Logic; }
      u32    GetNumSynthCmds() { return NumSynthCmds; }
      const string GetLogicName();
      const bool ExistsInvConstraintCmd();
      const bool ExistsSynthInvCmd();
      const bool ExistsNonExampleConstraints();
...
#endif /* __SYNTHLIB2_PARSER_IN_VISITOR_HPP */
